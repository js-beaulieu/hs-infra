...
test.describe('mcp flow', () => {
  test.describe('protected resource metadata (RFC 9728)', () => {
    test('metadata endpoint returns 200 with required fields', async ({ request }) => {
    ...
    test('metadata resource URI matches the configured MCP resource exactly', async ({ request }) => {
      const res = await request.get(`${MCP_METADATA}`);
      expect(res.status()).toBe(200);
      const json = await res.json();
      expect(json.resource).toBe(MCP_RESOURCE);
  ...
  test.describe('authorization server metadata (RFC 8414)', () => {
    test('AS metadata endpoint returns 200 with required fields', async ({ request }) => {
  ...
  test.describe('dynamic client registration (RFC 7591)', () => {
    test('DCR endpoint registers a new public MCP client and returns client_id', async ({ request }) => {
    ...
    test('DCR rejects registration with disallowed scope', async ({ request }) => {
      const res = await request.post(`${MCP_DCR}`, {
        headers: { 'Content-Type': 'application/json' },
        data: {
  ...
  test.describe('unauthenticated access', () => {
    test('MCP request without token returns 401 with WWW-Authenticate', async ({ request }) => {
  ...
  test.describe('token validation', () => {
    ...
    test('valid MCP token passes auth on GET (422 session required)', async ({ request }) => {
    ...
    test('valid MCP token initializes session via POST', async ({ request }) => {
      test.skip(!MCP_TOKEN_VALID, 'MCP_TOKEN_VALID not configured');
      const res = await request.post(`${MCP_RESOURCE}`, {
        headers: {
          Authorization: `Bearer ${MCP_TOKEN_VALID}`,
  ...
  test.describe('route boundary', () => {
    test('/mcpfoo does not match MCP route and has no MCP WWW-Authenticate', async ({ request }) => {
