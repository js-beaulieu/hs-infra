...
from tests.flows.flow_env import GENERATED_TOKENS_PATH, GENERATED_USERS_PATH, keycloak_origin_for
...
def kc_exec(cmd: str) -> str:
    ...
    keycloak_container = os.environ.get("KEYCLOAK_CONTAINER_NAME", "home-stack-keycloak-1")
    script = "\n".join(
        [
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Keycloak command failed: {cmd}\n{result.stderr or result.stdout}")
    return result.stdout


def create_user(username: str, password: str, email: str) -> str:
    print(f"Creating user {username}")
    kc_exec(f'"{KC}" create users -r "{REALM}" -s username="{username}" -s enabled=true -s email="{email}" -s emailVerified=true -s firstName="Flow" -s lastName="Test"')
    uid = kc_exec(f'"{KC}" get users -r "{REALM}" -q username="{username}" --fields id --format csv --noquotes | tail -n 1').strip()
    kc_exec(f'"{KC}" set-password --username "{username}" -r "{REALM}" --new-password "{password}" --temporary=false')
    return uid


def group_id(group: str) -> str:
    return kc_exec(f'"{KC}" get groups -r "{REALM}" -q search="{group}" --fields id,name --format csv --noquotes | grep ",{group}$" | cut -d, -f1 || true').strip()


def user_id(username: str) -> str:
    return kc_exec(f'"{KC}" get users -r "{REALM}" -q username="{username}" --fields id --format csv --noquotes | tail -n 1').strip()
...
def delete_protocol_mappers(client_id: str, cid: str) -> None:
    ...
    mappers = kc_exec(f'"{KC}" get clients/{cid}/protocol-mappers/models -r "{REALM}" --fields id,name --format csv --noquotes || true').strip()
    for line in mappers.splitlines():
        parts = line.split(",")
        name = ",".join(parts[1:])
        if name in {groups_mapper_name, audience_mapper_name}:
            try:
                kc_exec(f'"{KC}" delete clients/{cid}/protocol-mappers/models/{parts[0]} -r "{REALM}"')
            except Exception:
                pass


def create_ropc_client(client_id: str, client_secret: str, audience: str, access_token_lifespan_seconds: int | None = None) -> str:
    existing_id = kc_exec(f'"{KC}" get clients -r "{REALM}" -q clientId="{client_id}" --fields id --format csv --noquotes | tail -n 1 || true').strip()
    if existing_id:
    ...
    client_json = json.dumps(
        {
            ...
            "redirectUris": [],
            "attributes": {"access.token.lifespan": str(access_token_lifespan_seconds)} if access_token_lifespan_seconds else {},
        }
    )
    tmp_file = f"/tmp/mcp-test-client-{client_id}.json"
    kc_exec(f"cat > {tmp_file} <<'CLIENTJSON'\n{client_json}\nCLIENTJSON\n\"{KC}\" create clients -r \"{REALM}\" -f \"{tmp_file}\"\nrm -f {tmp_file}")
    cid = kc_exec(f'"{KC}" get clients -r "{REALM}" -q clientId="{client_id}" --fields id --format csv --noquotes | tail -n 1').strip()

    delete_protocol_mappers(client_id, cid)
    kc_exec(f'"{KC}" create clients/{cid}/protocol-mappers/models -r "{REALM}" -s name="{client_id}-groups" -s protocol=openid-connect -s protocolMapper=oidc-group-membership-mapper -s \'config."claim.name"=groups\' -s \'config."full.path"=true\' -s \'config."access.token.claim"=true\' -s \'config."id.token.claim"=true\' -s \'config."userinfo.token.claim"=true\'')
    kc_exec(f'"{KC}" create clients/{cid}/protocol-mappers/models -r "{REALM}" -s name="{client_id}-audience" -s protocol=openid-connect -s protocolMapper=oidc-audience-mapper -s \'config."included.custom.audience"={audience}\' -s \'config."access.token.claim"=true\' -s \'config."id.token.claim"=false\'')
    return cid


def create_wrong_aud_client(client_id: str, client_secret: str, wrong_audience: str) -> str:
...
def token_url() -> str:
    ...
    return urlunparse((parsed.scheme, parsed.netloc, f"/realms/{REALM}/protocol/openid-connect/token", "", "", ""))
...
def setup_keycloak_flow_state() -> None:
    ...
    for group in ["homelab-users", "tasks-users", "mcp-users"]:
        try:
            kc_exec(f'"{KC}" get groups -r "{REALM}" -q search="{group}" | grep \'"name" : "{group}"\'')
        except Exception:
    ...
    mcp_resource_uri = os.environ.get("MCP_RESOURCE") or os.environ.get("MCP_RESOURCE_URI", "")
    existing_tokens = {
    ...
    if mcp_resource_uri and not all(existing_tokens.values()):
        try:
            run_id_short = "".join(ch for ch in run_id if ch.isalnum() or ch == "-")[:12]
            valid_client_secret = os.environ.get("MCP_TEST_CLIENT_SECRET", f"mcp-valid-{run_id_short}")
            wrong_aud_client_secret = os.environ.get("MCP_WRONG_AUD_CLIENT_SECRET", f"mcp-wrong-aud-{run_id_short}")
            expired_client_secret = os.environ.get("MCP_EXPIRED_CLIENT_SECRET", f"mcp-expired-{run_id_short}")
            ...
            mcp_clients_to_cleanup = [valid_client_id, wrong_aud_client_id, expired_client_id]
            ...
            create_wrong_aud_client(wrong_aud_client_id, wrong_aud_client_secret, f"{mcp_resource_uri}-wrong")
            create_ropc_client(expired_client_id, expired_client_secret, mcp_resource_uri, -120)

            print("Generating MCP tokens via Keycloak token endpoint")
            generated_tokens["MCP_TOKEN_VALID"] = get_token(valid_client_id, valid_client_secret, mcp_user, mcp_pass)
            generated_tokens["MCP_TOKEN_MISSING_GROUP"] = get_token(valid_client_id, valid_client_secret, denied_user, denied_pass)
            generated_tokens["MCP_TOKEN_WRONG_AUD"] = get_token(wrong_aud_client_id, wrong_aud_client_secret, mcp_user, mcp_pass)
            generated_tokens["MCP_TOKEN_EXPIRED"] = get_token(expired_client_id, expired_client_secret, mcp_user, mcp_pass)
        finally:
    ...
    if mcp_resource_uri:
        missing_tokens = [key for key in existing_tokens if not generated_tokens.get(key)]
        if missing_tokens:
            raise RuntimeError(f"Missing MCP tokens after flow test setup: {', '.join(missing_tokens)}")
    ...
    if generated_tokens:
        GENERATED_TOKENS_PATH.write_text(json.dumps(generated_tokens, indent=2), encoding="utf-8")
    else:
        GENERATED_TOKENS_PATH.unlink(missing_ok=True)


def delete_user(username: str) -> None:
    uid = kc_exec(f'"{KC}" get users -r "{REALM}" -q username="{username}" --fields id --format csv --noquotes 2>/dev/null | tail -n 1 || true').strip()
    if uid:
...
def teardown_keycloak_flow_state() -> None:
    try:
        if not os.environ.get("KEYCLOAK_ADMIN_PASSWORD") or not GENERATED_USERS_PATH.exists():
            return
