...
test.describe("app login flow", () => {
  test("unauthenticated browser navigation reaches the login start flow, not tasks-api", async ({
    page,
  }) => {
  ...
  test("valid Keycloak user in /homelab-users and /tasks-users can authenticate", async ({
    browser,
  }) => {
    test.skip(
  ...
  test("authenticated browser returns to the frontend origin", async ({
    browser,
  }) => {
    test.skip(
      !TEST_USER_USERNAME || !TEST_USER_PASSWORD,
  ...
  test("oauth2-proxy cookie is Secure, HttpOnly, and scoped intentionally", async ({
    browser,
  }) => {
    test.skip(
      !TEST_USER_USERNAME || !TEST_USER_PASSWORD,
    ...
    expect(
      proxyCookie,
      "__Secure-oauth2_proxy cookie should exist",
    ).toBeTruthy();
    expectCookieSecureHttpOnly(proxyCookie!);
    await context.close();
  });

  test("user missing /tasks-users is denied app/API access after authentication", async ({
    browser,
  }) => {
    test.skip(
      !TEST_DENIED_USER_USERNAME || !TEST_DENIED_USER_PASSWORD,
      "Denied test user credentials not configured",
    );
    const context = await browser.newContext();
    const page = await context.newPage();
    await navigateToLogin(page, WEB_ORIGIN);
    ...
    await page.waitForURL((url) => url.toString().startsWith(WEB_ORIGIN), {
      timeout: 15000,
    });
    // Expect a denial: 403 or an error page
    const status = (await page.evaluate(
      () =>
        (window as any).performance?.getEntriesByType("navigation")[0]
          ?.responseStatus,
    )) as number | undefined;
  ...
  test("expired or invalid session cookie is rejected", async ({ browser }) => {
        ...
        cookies: [
          {
            ...
            secure: pc.secure,
            sameSite: pc.sameSite as "Lax" | "Strict" | "None",
          },
        ],
        origins: [],
      },
    ...
    expect(
      url.includes("/oauth2/start") ||
        url.includes("/realms/homelab/protocol/openid-connect/auth"),
