...
def test_protected_api_without_valid_session_returns_401_or_403_for_exact_api(
    http_client, flow_env
):
...
def test_inbound_authorization_header_is_stripped_on_normal_browser_api(
    ...
    res = http_client.get(
        f"{flow_env.api_base}/users/me", headers={"Authorization": "Bearer fake-token"}
    )
    ensure_no_auth_redirect_status(res.status_code)


def test_unsafe_methods_without_valid_origin_or_referer_are_rejected_before_proxying(
    http_client, flow_env
):
...
def test_api_responses_from_allowed_origin_include_credentialed_cors_headers(
    http_client, flow_env
):
    res = http_client.get(
        f"{flow_env.api_base}/users/me", headers={"Origin": flow_env.web_origin}
    )
    ensure_no_auth_redirect_status(res.status_code)
    assert res.headers.get("access-control-allow-origin") == flow_env.web_origin
    assert res.headers.get("access-control-allow-credentials") == "true"
...
def test_options_preflight_from_untrusted_origin_is_rejected_or_not_permissive(
    http_client, flow_env
):
    res = http_client.request(
        "OPTIONS",
...
def test_unsafe_method_from_allowed_origin_is_allowed_only_when_authenticated(
    browser, flow_env
):
    ...
    try:
        res = context.request.post(
            f"{flow_env.api_base}/users/me",
            headers={"Origin": flow_env.web_origin},
            data={},
        )
