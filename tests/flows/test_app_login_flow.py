...
from tests.flows.flow_helpers import (
    expect_cookie_secure_http_only,
    navigate_to_login,
    perform_keycloak_login,
)


def test_unauthenticated_browser_navigation_reaches_the_login_start_flow_not_tasks_api(
    page, flow_env
):
...
def test_oauth2_proxy_cookie_is_secure_httponly_and_scoped_intentionally(
    browser, flow_env
):
    context = browser.new_context(ignore_https_errors=True)
    try:
...
def test_user_missing_tasks_users_is_denied_app_api_access_after_authentication(
    browser, flow_env
):
    context = browser.new_context(ignore_https_errors=True)
    try:
        ...
        is_denied = status == 403 or (
            ...
            and (
                "403" in body_text
                or "forbidden" in body_text.lower()
