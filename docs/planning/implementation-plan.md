...
- Public issuer: `https://auth.${DOMAIN}/realms/home-stack`.
...
- First app API: private `tasks-api` service exposed publicly through `https://api.${DOMAIN}/tasks`. Normal browser API routes such as `/tasks/users/me` strip `/tasks` and proxy to `tasks-api`. agentgateway exposes public MCP at `/tasks/mcp` and proxies to the upstream `tasks-api` MCP endpoint.
...
- `https://auth.${DOMAIN}/realms/home-stack/clients-registrations/openid-connect`

MCP resource identity for the first app:

- Canonical MCP resource URI: `https://api.${DOMAIN}/tasks/mcp`.
- Protected resource metadata URL: `https://api.${DOMAIN}/tasks/.well-known/oauth-protected-resource/mcp`.
- Protected resource metadata must return `resource` exactly equal to `https://api.${DOMAIN}/tasks/mcp` and `authorization_servers` containing `https://auth.${DOMAIN}/realms/home-stack`.
- Unauthenticated MCP responses must include `WWW-Authenticate: Bearer resource_metadata="https://api.${DOMAIN}/tasks/.well-known/oauth-protected-resource/mcp"` or the equivalent valid RFC 9728 challenge generated by agentgateway.
- MCP access tokens must be audience-bound to the MCP resource. Prefer validating `aud` contains `https://api.${DOMAIN}/tasks/mcp`. If agentgateway requires a non-URI audience such as `tasks-mcp`, document that choice and make the Keycloak audience mapper and agentgateway validation match exactly.
- Create a Keycloak client scope or dedicated mapper that adds the MCP audience to access tokens used for `/tasks/mcp`. For dynamically registered MCP clients, ensure registration policy or post-registration setup does not issue tokens missing the MCP audience.
...
## Required Route Precedence For `api.${DOMAIN}/tasks`
...
Do not use `/tasks/mcp*`; it matches unwanted paths like `/tasks/mcpfoo`.
...
- Route `api.${DOMAIN}/oauth2/*` directly to the shared oauth2-proxy.
...
- Route exact `/tasks` and `/tasks/*` through an oauth2-proxy auth check, strip `/tasks`, then to `tasks-api`.
...
- For canonical API routes on `api.${DOMAIN}/tasks`, use the same auth check but return `401/403`, not a browser login redirect.
...
`api.${DOMAIN}/tasks` is browser-authenticated with cookies for normal app API routes. Because the app has unsafe methods (`POST`, `PUT`, `PATCH`, `DELETE`), add gateway-level CSRF protection for browser API routes:
...
- Enforce `/tasks-users` at Caddy or a gateway authz layer for `tasks.${DOMAIN}` and `api.${DOMAIN}/tasks`. A single shared oauth2-proxy instance should not become the only app-specific authorization layer unless all apps share the same group policy.

For the first app, register the canonical callback URI:

- `https://api.${DOMAIN}/oauth2/callback`
...
- Create or import the `home-stack` realm.
...
- Keep the public issuer fixed as `https://auth.${DOMAIN}/realms/home-stack` even when containers fetch metadata internally.
...
- `oauth2-proxy-tasks`: confidential OIDC client for browser auth, authorization code flow, PKCE S256, redirect URI `https://api.${DOMAIN}/oauth2/callback`.
- `tasks-mcp`: public OIDC client for initial local MCP testing, plus MCP audience/client-scope configuration that issues access tokens valid for `https://api.${DOMAIN}/tasks/mcp`.
...
- Give agentgateway a way to resolve/fetch Keycloak issuer metadata and JWKS for the public issuer `https://auth.${DOMAIN}/realms/home-stack`.
- Configure route matches for exact `/tasks/mcp` and prefix `/tasks/mcp/*`.
- Configure an exact protected-resource metadata route for `/tasks/.well-known/oauth-protected-resource/mcp`.
- If using agentgateway authorization-server proxy mode, configure exact routes for `/tasks/.well-known/oauth-authorization-server/mcp` and `/tasks/.well-known/oauth-authorization-server/mcp/client-registration`.
...
- Proxy MCP traffic to the same `tasks-api` service that serves the API.
...
- Set protected resource metadata `resource` to `https://api.${DOMAIN}/tasks/mcp`; include authorization server `https://auth.${DOMAIN}/realms/home-stack`, supported scopes, and `bearerMethodsSupported` with `header`.
...
- `keycloak-bootstrap`: attached to `auth`, no host ports, idempotently creates the `home-stack` realm, baseline groups, OIDC clients, and protocol mappers after Keycloak is healthy.
...
Implement the `tasks.${DOMAIN}` frontend and canonical `api.${DOMAIN}/tasks` API route.
...
- `GET https://api.${DOMAIN}/tasks/something` without valid browser/session auth returns `401/403`, not `302`.
- `GET https://api.${DOMAIN}/tasks/something` with valid auth reaches `tasks-api` with trusted identity headers.
- Unsafe browser API methods without same-origin CSRF signals return `403`.
- `GET https://api.${DOMAIN}/tasks/.well-known/oauth-protected-resource/mcp` returns MCP protected resource metadata with exact `resource: https://api.${DOMAIN}/tasks/mcp`.
- `POST/GET https://api.${DOMAIN}/tasks/mcp` uses agentgateway MCP auth behavior, not oauth2-proxy browser redirect behavior.
...
- Use the real protected user route for the initial slice, `GET https://api.${DOMAIN}/tasks/users/me`.
- From the logged-in browser app, call `GET https://api.${DOMAIN}/tasks/users/me` with credentials and confirm it reaches `tasks-api` with gateway-derived `X-User-ID`, `X-User-Name`, and `X-User-Email` populated from the trusted `X-Auth-*` contract.
- Confirm the returned subject is the Keycloak `sub`, not a mutable username or email.
- Confirm unauthenticated `GET https://api.${DOMAIN}/tasks/users/me` returns `401/403`, not a browser `302` login redirect.
- Confirm a spoof attempt such as `curl -i -H 'X-Auth-Subject: attacker' https://api.${DOMAIN}/tasks/users/me` does not pass the spoofed value to `tasks-api`.
...
- Confirm `/tasks`, `/tasks/*`, `/tasks/mcp`, and `/tasks/mcp/*` are distinct by testing `/tasks/mcpfoo`; it must not route to MCP.
...
- Confirm `GET https://api.${DOMAIN}/tasks/.well-known/oauth-protected-resource/mcp` returns public protected-resource metadata with `resource` exactly `https://api.${DOMAIN}/tasks/mcp` and `authorization_servers` containing `https://auth.${DOMAIN}/realms/home-stack`.
...
- Call the MCP initialize flow through `https://api.${DOMAIN}/tasks/mcp` and confirm agentgateway validates the token and proxies to `tasks-api`.
...
- Confirm browser SSO cookies alone do not authorize `/tasks/mcp`; MCP must use bearer auth through agentgateway.
- Confirm AI/MCP traffic never receives oauth2-proxy browser redirects.

### Keycloak And Edge Verification

- Confirm `https://auth.${DOMAIN}/realms/home-stack/.well-known/openid-configuration` is reachable publicly and reports issuer `https://auth.${DOMAIN}/realms/home-stack`.
...
- `curl -i https://api.${DOMAIN}/tasks` returns `401/403`, not `302`, when unauthenticated.

- `curl -i https://api.${DOMAIN}/tasks/something` returns `401/403`, not `302`, when unauthenticated.

- `curl -i https://api.${DOMAIN}/tasks/mcp` returns MCP auth behavior, not `302`, when unauthenticated.

- `curl -i https://api.${DOMAIN}/tasks/mcpfoo` does not match the MCP route.

- `curl -i https://api.${DOMAIN}/tasks/.well-known/oauth-protected-resource/mcp` returns public MCP protected resource metadata, not a browser redirect.

- MCP protected resource metadata contains `resource` exactly equal to `https://api.${DOMAIN}/tasks/mcp`.
...
- From oauth2-proxy and agentgateway containers, fetching `https://auth.${DOMAIN}/realms/home-stack/.well-known/openid-configuration` succeeds and the returned issuer is `https://auth.${DOMAIN}/realms/home-stack`.
...
- Authenticated MCP token without `/mcp-users` cannot access `/tasks/mcp`.
