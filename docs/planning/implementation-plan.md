...
- Protected resource metadata must return `resource` exactly equal to `https://api.tasks.${DOMAIN}/mcp` and `authorization_servers` containing `https://auth.${DOMAIN}/realms/home-stack`.
...
- Set protected resource metadata `resource` to `https://api.tasks.${DOMAIN}/mcp`; include authorization server `https://auth.${DOMAIN}/realms/home-stack`, supported scopes, and `bearerMethodsSupported` with `header`.
...
- Confirm `GET https://api.tasks.${DOMAIN}/.well-known/oauth-protected-resource/mcp` returns public protected-resource metadata with `resource` exactly `https://api.tasks.${DOMAIN}/mcp` and `authorization_servers` containing `https://auth.${DOMAIN}/realms/home-stack`.
