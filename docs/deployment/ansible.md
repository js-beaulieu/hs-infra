...
ansible/playbooks/bootstrap.yml
  Manual first run on a fresh VPS.
  Installs python3/sudo, creates the deploy user with SSH keys.

ansible/playbooks/site.yml
  CI host convergence as the deploy user (with become).
  Installs base packages, Docker, UFW, fail2ban, SSH hardening, and the deploy wrapper.

ansible/playbooks/deploy.yml
  Routine app deployment as the deploy user.
  Updates the source checkout, renders .env, calls /usr/local/sbin/home-stack-deploy, and checks health.
```

In production, `/opt/home-stack` is root-owned. The deploy wrapper checks out the requested version as root and runs Docker Compose as root so Caddy can bind `80/443`.
...
home_stack_cloudflare_api_token: ...
```
